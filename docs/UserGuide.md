...
Edits a specified transaction from a contact

Format : `editTxn i/PERSON_INDEX t/TRANSACTION_INDEX [n/NAME] [a/AMOUNT]`

* Edits the transaction specified by `TRANSACTION_INDEX` for the person at the specified `PERSON_INDEX`.
* At least one of the optional fields must be provided.
* Existing values will be updated to the input values.
* The index **must be a positive integer** 1, 2, 3, …​
